 ...
 */
class EuclidianMapping(val startPoint: Coordinate) {

  lazy val (dlat, dlon) = CoordinateGeometry.localGeometry(startPoint)

  def map(p: Coordinate): EuclidianPoint = {
    val (dlat0, dlon0) = CoordinateGeometry.difference(startPoint, p)
    val (x1, y1) = (dlat0 * dlat, dlon0 * dlon)
