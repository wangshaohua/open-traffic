package edu.berkeley.path.bots.netconfig.Datum;
...
public class PathInference<LINK extends Link> implements Serializable {
    ...
    public <LINK2 extends Link> PathInference<LINK2> clone(
        ...
        return new PathInference<LINK2>(id(), startTime(), endTime(),
                new_routes, new_probabilities, hired());
    }
    ...
    public RouteTT<LINK> getMostProbableRouteTT() throws NetconfigException {
        ...
        if (null == ret) {
            ...
            return RouteTT.from(ret, this.startTime(), this.endTime(),
                    this.id(), this.hired());
        }
    }
    ...
    public RouteTT<LINK> toRouteTT() throws NetconfigException {
        ...
        return RouteTT.from(this.routes().get(0), this.startTime(),
                this.endTime(), this.id(), this.hired());
    }
    ...
    public String toString() {
        ...
        sb.append(String.format("%s from %s to %s .get(\n", this.id(), this
                .startTime().toString(), this.endTime().toString()));
        for (int i = 0; i < this.routes().size(); i++) {
            sb.append(String.format("\t\t%f\t%s\n)", this.probabilities()
                    .get(i), this.routes().get(i).toString()));
        }
        return sb.toString();
    } // toString.
